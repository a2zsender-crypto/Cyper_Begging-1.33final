...
import { useTranslation } from 'react-i18next'; // [UPDATE] Import để lấy ngôn ngữ
...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
        // [UPDATE 3] Gửi request kèm user_id và language
        const { data, error } = await supabase.functions.invoke('contact-handler', {
            body: { 
                ...
                user_id: session?.user?.id || null, // Lấy ID user nếu đã login
                language: i18n.language // Lấy ngôn ngữ hiện tại ('vi' hoặc 'en')
            }
        });
    ...
    <div className="container mx-auto p-4 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          {/* [UPDATE 4] Bảng Lịch sử Hỗ trợ (Chỉ hiện khi login) */}
          {showHistory && (
             <div className="bg-white p-6 shadow rounded-lg">
