...
export default function Cart() {
  ...
  const renderVariants = (variantsObj) => {
      ...
      // Ẩn các trường hệ thống bắt đầu bằng _
  ...
  const handleCheckout = async () => {
      // SỬA: Đa ngôn ngữ cho các alert
      if (!formData.name || !formData.email) return alert(t('Vui lòng điền Tên và Email', 'Please fill in Name and Email'));
      if (hasPhysical && (!formData.phone || !formData.address)) return alert(t('Vui lòng điền địa chỉ giao hàng', 'Please fill in shipping address'));
      ...
      try {
          if (!user && isRegister) {
              ...
              if (authError) {
                  if (authError.message.includes('already registered')) {
                      alert(t('Email này đã có tài khoản. Thanh toán như khách.', 'Email already registered. Checking out as guest.'));
                  } else {
          ...
          // Phải dùng JSON.stringify cho body
          const { data, error } = await supabase.functions.invoke('payment-handler', {
              ...
              method: 'POST', // Đảm bảo method là POST
              headers: { 'Content-Type': 'application/json' }
          });

          if (error) {
              // Parse error body nếu có để hiển thị chi tiết hơn
              let errorMsg = error.message;
              try {
                 const errBody = JSON.parse(error.message); // Đôi khi supabase trả JSON trong message
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            {cart.map((item, idx) => (
                <div key={item.cartItemId || `${item.id}-${idx}`} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex gap-4 items-center">
                    ...
                    <button onClick={()=>removeFromCart(item.cartItemId || item.id)} className="text-red-400 hover:text-red-600 p-2"><Trash2 size={20}/></button>
                </div>
            ))}
        </div>
