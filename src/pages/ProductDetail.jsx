...
export default function ProductDetail() {
  ...
  const [product, setProduct] = useState({
    ...
    is_digital: true,
    images: [],
    variants_config: [] // Mảng chứa cấu hình: { name, code, price }
  });
  ...
  const fetchProduct = async () => {
    ...
    // Fallback nếu variants_config null
    if (!data.variants_config) data.variants_config = [];
    setProduct(data);
  };
  ...
  const addVariant = () => {
      setProduct({
          ...product,
          variants_config: [...product.variants_config, { name: '', code: '', price: 0 }]
      });
  };

  const removeVariant = (index) => {
      const newVars = [...product.variants_config];
      newVars.splice(index, 1);
      setProduct({ ...product, variants_config: newVars });
  };
  ...
  const updateVariant = (index, field, value) => {
      ...
      // Tự động sinh Code nếu đang nhập Name và Code chưa được nhập tay
  ...
  const handleSave = async (e) => {
    ...
    // Chuẩn hóa dữ liệu variants trước khi lưu
    const cleanVariants = product.variants_config.map(v => ({
        ...v,
        code: v.code || generateSlug(v.name).toUpperCase() // Đảm bảo luôn có code
    }));

    const payload = { ...product, variants_config: cleanVariants };
    delete payload.id; 
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20 animate-fade-in">
        ...
        <form onSubmit={handleSave} className="grid grid-cols-3 gap-8">
            ...
            <div className="col-span-2 space-y-6">
                ...
                {/* Phần cấu hình biến thể (MỚI THÊM) */}
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-4">
                    ...
                    {product.variants_config.length === 0 ? (
                        <p className="text-sm text-slate-400 italic text-center py-4">
                            {t('Sản phẩm này chưa có biến thể (Bán theo giá gốc).', 'No variants configured (Use default price).')}
                        </p>
                    ) : (
                        <div className="space-y-3">
                            <div className="grid grid-cols-12 gap-2 text-xs font-bold text-slate-500 uppercase bg-slate-50 p-2 rounded">
                                <div className="col-span-5">{t('Tên hiển thị', 'Display Name')}</div>
                                <div className="col-span-4">{t('Mã SKU (Auto)', 'SKU Code (Auto)')}</div>
                                <div className="col-span-2">{t('Giá riêng', 'Price Override')}</div>
                                <div className="col-span-1"></div>
                            </div>
                            {product.variants_config.map((v, idx) => (
                                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                                    <div className="col-span-5">
                                        <input 
                                            placeholder={t("VD: Viettel 50k", "Ex: Size XL")}
                    ...
                    {product.variants_config.length > 0 && (
                        <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-100 flex gap-2 items-start text-xs text-yellow-700 mt-2">
                            ...
                            <p>
                                {t(
                                    'Mã SKU sẽ dùng để khớp với kho Key và API. Hệ thống tự động gợi ý mã khi bạn nhập tên, nhưng bạn có thể sửa lại theo ý muốn (VD: VTT10).',
                                    'SKU Code is used for Key matching and API. System auto-generates it, but you can edit manually (e.g., VTT10).'
                                )}
                            </p>
                        </div>
                    )}
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                   <button disabled={loading} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg shadow-
