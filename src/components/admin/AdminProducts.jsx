  ...
  const handleImportStock = async () => {
    try {
        ...
        if (currentProd.is_digital) {
            ...
            if (isNaN(qtyToAdd) || qtyToAdd <= 0) return toast.warn(t("Số lượng > 0", "Quantity > 0"));
         ...
         <table className="w-full text-left">
           <thead className="bg-slate-50 border-b text-slate-500 text-xs uppercase font-bold tracking-wider">
               <tr>
                   ...
                   <th className="p-4">{t('Loại', 'Type')}</th>
                   <th className="p-4">{t('Chế độ API', 'API Mode')}</th>
                   <th className="p-4">{t('Tồn kho', 'Stock')}</th>
                   <th className="p-4">{t('Thao tác', 'Action')}</th>
               </tr>
           </thead>
           ...
           <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-auto animate-scale-in">
              <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl font-bold text-slate-800">
                      {productForm.id ? t('Sửa sản phẩm', 'Edit Product') : t('Thêm sản phẩm mới', 'Add New Product')}
                  </h2>
              ...
              <form onSubmit={handleSaveProduct} className="space-y-6">
                 ...
                 {productForm.is_digital && (
                     <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-start gap-3">
                         ...
                         <div>
                             <label htmlFor="apiKeyToggle" className="block text-sm font-bold text-blue-800">{t('Lấy Key qua API khi hết hàng', 'Get Key over API if Out of Stock')}</label>
                 ...
                 <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
                    <div className="flex justify-between items-center mb-3">
                        <label className="block text-sm font-bold text-slate-700 flex items-center gap-2"><Settings size={16}/> {t('Biến thể sản phẩm', 'Product Options (Variants)')}</label>
                    ...
                    {productForm.is_digital && productForm.variant_stocks.length > 0 && (
                        <div className="mt-6 border-t pt-4">
                             <label className="block text-sm font-bold text-slate-700 mb-2">{t('Quản lý Key theo biến thể', 'Manage Keys per Variant')}</label>
                    ...
                    {!productForm.is_digital && productForm.variant_stocks.length > 0 && (
                        <div className="mt-6 border-t pt-4">
                            <h4 className="font-bold mb-3 text-slate-700 flex items-center gap-2"><Layers size={16}/> {t('Tồn kho biến thể vật lý', 'Physical Variant Stock')}</h4>
                            <div className="bg-orange-50/50 rounded-xl border border-orange-100 p-4 space-y-2">
                                    ...
                                    <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-lg border border-orange-100 shadow-sm">
                                        <div className="text-sm font-medium text-slate-700">
                                            {Object.entries(item.options).map(([k, v]) => (
                                                <span key={k} className="mr-2 px-2 py-1 bg-orange-50 text-orange-700 rounded text-xs border border-orange-100 font-bold">{v}</span>
                                            ))}
                                        </div>
                                ...
                                <div className="text-right text-xs text-slate-500 italic mt-2">
                                    *Tổng tồn kho sẽ tự động cập nhật: <strong>{productForm.physical_stock}</strong>
                                </div>
                            </div>
                        </div>
                    )}
                 ...
                 {productForm.is_digital && (!productForm.variants || productForm.variants.length === 0) && (
                     <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 flex justify-between items-center">
                         <div>
                            <label className="block text-sm font-bold text-purple-800 mb-1">{t('Kho số (Không biến thể)', 'Digital Stock (No Variants)')}</label>
                 ...
                 {!productForm.is_digital && (
                     <div className="bg-orange-50 p-4 rounded-xl border border-orange-100">
                         <label className="block text-sm font-bold text-orange-800 mb-1">{t('Tổng tồn kho vật lý', 'Physical Stock (Total)')}</label>
                 ...
                 <div>
                    <label className="block text-sm font-bold mb-2 text-slate-600">{t('Hình ảnh', 'Images')}</label>
