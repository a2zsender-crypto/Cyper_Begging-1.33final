...
export default function AdminContacts({ session, role, activeTicketId, onNewTicket }) {
  ...
  // Lấy tên shop để hiển thị thay cho chữ ADMIN
          ...
          .on('postgres_changes', { event: '*', schema: 'public', table: 'contacts' }, payload => {
              if(payload.eventType === 'INSERT') {
                 if (role === 'admin' || (role === 'user' && payload.new.email === session.user.email)) {
                    ...
                    if(role === 'admin') toast.info("Có yêu cầu hỗ trợ mới!");
                 }
  ...
  const handleSendReply = async (e) => {
      ...
      try {
          const { error } = await supabase.from('contact_replies').insert({
              contact_id: showTicketModal.id, sender_role: role, message: replyMessage
          });
  ...
  // HÀM XỬ LÝ KHI BẤM NÚT NEW TICKET
  const handleNewTicketClick = () => {
      if (onNewTicket) {
          onNewTicket(); // Gọi hàm của cha (Contact.jsx) để tắt history
      } else {
          navigate('/contact');
      }
  };
    ...
    <div className="animate-fade-in h-full flex flex-col">
       ...
       <div className="bg-white shadow-sm rounded-xl overflow-hidden border border-slate-200 flex-1 overflow-y-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b text-slate-500 text-xs uppercase font-bold tracking-wider sticky top-0"><tr><th className="p-4">Date</th><th className="p-4">{role==='admin'?'Customer':'Subject'}</th><th className="p-4">Status</th><th className="p-4">Action</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {contacts.length > 0 ? contacts.map(c => (
                <tr key={c.id} className={`hover:bg-slate-50 transition cursor-pointer ${c.status === 'new' ? 'bg-blue-50/30' : ''}`} onClick={() => openTicketChat(c)}>
                    ...
                    <td className="p-4">{role === 'admin' ? <div><span className="font-bold text-slate-700">{c.name}</span><div className="text-xs text-slate-400">{c.email}</div></div> : <div className="font-medium text-slate-700 truncate max-w-xs">{c.message}</div>}</td>
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[999] p-4 backdrop-blur-sm">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl h-[85vh] flex flex-col overflow-hidden animate-scale-in">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                      {/* TIN NHẮN GỐC (Sửa logic hiển thị: Nếu là User -> Nằm phải/Xanh. Admin -> Trái) */}
                      <div className={`flex ${role === 'user' ? 'justify-end' : 'justify-start'}`}>
                          <div className={`p-3 rounded-2xl max-w-[85%] shadow-sm text-sm leading-relaxed ${role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border border-slate-200 rounded-tl-none text-slate-800'}`}>
                             <p className="text-[10px] font-bold opacity-70 mb-1 uppercase">{role === 'user' ? 'You' : showTicketModal.name}</p>
                      ...
                      {ticketReplies.map(r => {
                          ...
                          const displayName = r.sender_role === 'admin' ? shopName : (role === 'admin' ? showTicketModal.name : 'You');
