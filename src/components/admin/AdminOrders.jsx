...
import { useQuery } from '@tanstack/react-query'; // Import mới
...
export default function AdminOrders({ session, role }) {
  ...
  const { data: orders = [], isLoading } = useQuery({
    queryKey: ['admin-orders', role, session?.user?.email], // Key phụ thuộc vào role/email
    queryFn: async () => {
    ...
    enabled: !!session // Chỉ fetch khi có session
  });
