...
serve(async (req) => {
  ...
  try {
    ...
    for (const item of items) {
        ...
        // --- LOGIC TRỪ KHO CHUẨN XÁC ---
        if (!product.is_digital) {
             let newPhysicalStock = product.physical_stock;
             ...
             if (product.variant_stocks && product.variant_stocks.length > 0 && item.selectedVariants) {
                 ...
                 if (variantIndex === -1) {
                     // Trường hợp lỗi dữ liệu: Khách mua variant không tồn tại -> Bỏ qua hoặc báo lỗi
                     // Ở đây ta vẫn cho qua nhưng không trừ kho variant (fallback)
                 } else {
                     const currentVarStock = parseInt(product.variant_stocks[variantIndex].stock);
                     if (currentVarStock < item.quantity) {
                         throw new Error(`Sản phẩm "${product.title}" (${JSON.stringify(item.selectedVariants)}) không đủ hàng!`);
                     }
                     ...
                     // Tính lại tổng kho
                     newPhysicalStock = product.variant_stocks.reduce((sum: number, v: any) => sum + (parseInt(v.stock)||0), 0);
                     
                     // Cập nhật DB ngay lập tức (Lưu ý: Concurrent requests cao có thể conflict, nhưng chấp nhận được ở mức shop vừa)
             ...
             // 2. Nếu sản phẩm không có biến thể (Logic cũ)
             else {
                 if (product.physical_stock < item.quantity) {
                     throw new Error(`Sản phẩm "${product.title}" không đủ hàng!`);
                 }
                 await supabase.from('products').update({ 
                     physical_stock: product.physical_stock - item.quantity 
                 }).eq('id', product.id);
             }
        }
